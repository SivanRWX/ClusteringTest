...
# DBSCAN将具有足够密度的区域划分为簇，并在具有噪声的空间数据库中发现任意形状的簇，它将簇定义为密度相连的点的最大集合。
# 是基于一组邻域来描述样本集的紧密程度的，参数(eps, min_samples)用来描述邻域的样本分布紧密程度。
# 其中，eps描述了某一样本的邻域距离阈值，min_samples描述了某一样本的距离为eps的邻域中样本个数的阈值[1]
