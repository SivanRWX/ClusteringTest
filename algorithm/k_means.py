...
def k_means_clustering(data, features, best_k):
    ...
    for _ in range(100):  # 最大迭代次数
        # 计算每个点到各聚类中心的距离（二维数组）
        # np.linalg.norm 用于计算向量的范数（默认是欧几里得范数）
        # 通过广播机制，X[:, np.new axis] 和 centers 进行逐元素相减，得到一个形状为 (n_samples, best_k, n_features) 的数组
        distances = np.linalg.norm(X[:, np.newaxis] - centers, axis=2)

        # 分配每个点到最近的聚类中心（一维数组）
        # 根据每个数据点到各聚类中心的距离，将数据点分配到最近的聚类中心，并生成一个包含所有数据点聚类标签的数组 kmeans_label
